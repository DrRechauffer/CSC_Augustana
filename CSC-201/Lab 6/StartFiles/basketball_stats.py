...
Document Assistance given or received after class period ends: none
...
def get_team_average(points):
    ...
    pass
    team_total = 0 
    for row_score in range(len(points)):
        for col_score in range(len(points[row_score])):
            team_total = team_total + points[row_score][col_score]
    team_average = round(float((team_total / len(points))), 2)
    print("Team's average points per game is %.2f" %team_average)
...
def print_game_points(points):
    ...
    print('Game   Points')
    for gameNum in range(len(points)):
        print(' ', gameNum, '    ', sum(points[gameNum]))
